...
/**********************************************/
/* Step 1: Convervsation Information recovery */
/**********************************************/
if(!PREVIOUS1)
{
...
float SnI[2][2],vec1[2],vec2[2];
float eigenvector1[2],eigenvector2[2],eigenvalues[2];
float ave_error,st_dev,min_angle,max_angle,density;
...
for(j=OFFSET_X+EXTRA_OFFSET_X;j<pic_x-OFFSET_X;j++) 
	...
	*/
	...
	if(L2norm(diff,2) > DIFF_THRESH) RELAXATION = TRUE;
	}
}
...
for(v=(-size);v<=size;v++)
	...
	if(u >= -N && u <= N && v >= -N && v <= N)
	{
	if(SSDvalues[u_index][v_index] < sum_min) 
		{ sum_min=SSDvalues[u_index][v_index]; a=u; b=v; }
